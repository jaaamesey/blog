...
<DemoWrapper>
<EmbedReactComponent loader={() => import('./react_bad.react.jsx').then(r=>r.BadTodoAppWithKey)}/>
</DemoWrapper>
...
In my opinion, these workarounds are a lot more complex than just having proper tools for mutable derived state. [Svelte](https://svelte.dev/docs/svelte/$derived) and [SolidJS](https://dev.to/this-is-learning/mutable-derivations-in-reactivity-2ffl) seem to agree.
...
But React implementations of this concept do exist, such as [use-state-with-deps](https://react.dev/reference/react/useState#storing-information-from-previous-renders):
