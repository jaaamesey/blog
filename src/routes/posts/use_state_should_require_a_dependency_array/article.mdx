...
function Component({ someProp }) {
	// ESLint error: initial state depends on a mutable function Component({ someProp }) {value
	const [state, setState] = useState(someProp);
	...
}
```

The second is that **derived state is *sometimes* a valid approach to solving a problem**, but **the escape hatches React suggests can be somewhat invasive and error-prone**.
...
**Try changing the brush colour before editing another task.** Why should the colour we're drawing with reset each time we change what task we're editing?
...
No messing with `key`, setter calls in the root render method, or `useEffect` - it just kinda works.
