...
React has a wonderful hook called [`useState`](https://react.dev/reference/react/useState) that we use to store things that might change over time, and have those changes reflected in the UI.

There's a few points I'd like to argue here.

My first, and hopefully least controversial point, is that **using `useState` like this should throw a lint error**:
...
It's the cases that can't be represented this way - cases that need *mutable* derived state - that get really messy. This tends to come up a lot when dealing with user input and optimistic updates (if you can't use `useOptimistic` or Tanstack Query for whatever reason).

Let's go back to the todo list example from before. The textbook way of fixing this is through the `key` prop, which exists on every React component.

And it works! But we have to be careful with this - what if there's still some state that we want to *preserve*?

If `key` changes between renders, the entire component tree below that point is re-mounted. In addition to DOM nodes being recreated, any state declared below that point is *trashed* and reset.

We can't just reset state at the level of a single `useState` hook, or even a single component instance. Also, you need to create a proper `key`, which can get annoying when working with more than one dependency.

So, sometimes forcing a remount with `key` is fine, but there's many cases when it's too much of an ask. 

For those cases, the React docs provides a more isolated pattern:
...
The nicest thing I can say about this is that it works. And at the very least, React *does* prevent Effects from running and children from re-rendering if all a render here does is trigger another render.

But this looks confusing, despite this pattern only being applied to *one* value. We now have to manage previous state, and there's a lot of room for things to go wrong. Maybe our condition gets us stuck in an infinite loop, or maybe we accidentally omit a dependency that we should be listening to.

What I've seen more in practice is code like this, which definitely *looks* more elegant:
...
[Svelte](https://svelte.dev/docs/svelte/$derived) and [SolidJS](https://dev.to/this-is-learning/mutable-derivations-in-reactivity-2ffl) are now introducing proper tools for dealing with mutable derived state.
...
But React implementations of this concept do exist, such as https://github.com/peterjuras/use-state-with-deps:

EXAMPLE

Hopefully the somewhat inflammatory title of this article makes a bit more sense now. What I'm proposing is that this becomes **how `useState` works by default**. 
...
There's a lot of precedent to draw from with how dependency arrays for `useMemo` and `useEffect` work. We'd probably want to inherit the same ESLint warnings and autofixes, and have the React Compiler auto-generate it in the same way.
...
...which is a much simpler escape hatch than what it takes to get the behaviour that's more "correct" today.
