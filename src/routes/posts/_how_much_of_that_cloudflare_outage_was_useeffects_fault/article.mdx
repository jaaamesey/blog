>TL;DR:
>1. I'd say it's React's rendering model that leads to this specific problem with `useEffect`, more than the core `useEffect` API or concept of an "effect" itself
...
To be fair, React would likely share this problem with fully immediate mode renderers like [imgui](https://github.com/ocornut/imgui). But, I don't think we put widgets in that domain under the stress of managing so much state and side effects themselves. 
